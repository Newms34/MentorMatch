...
"use strict";!function(){r.$inject=["$timeout"];const e=io(),t=angular.module("codementormatch-app",["ui.router","ngAnimate","ngSanitize","ngMaterial","ngMessages"]),s=angular.module("reset-app",[]);Array.prototype.findUser=function(e){for(var t=0;t<this.length;t++)if(this[t].user==e)return t;return-1};t.constant("IsDevelopment","localhost"===window.location.hostname).config(["$stateProvider","$urlRouterProvider","$locationProvider","$httpProvider","$compileProvider","$logProvider","IsDevelopment","$mdGestureProvider",function(t,o,s,n,r,a,i,l){s.html5Mode(!0),o.otherwise("/404"),l.skipClickHijack(),r.debugInfoEnabled(i),a.debugEnabled(i),i&&console.log("-------------------------\nDebug mode enabled \n-------------------------"),t.state("app",{abstract:!0,templateUrl:"layouts/full.html"}).state("app.dash",{url:"/",templateUrl:"components/dash.html"}).state("app.mentor",{url:"/mentor",templateUrl:"components/mentor.html"}).state("app.mod",{url:"/mod",templateUrl:"components/mod.html"}).state("app.help",{url:"/help",templateUrl:"components/help/help.html"}).state("app.match",{url:"/match",templateUrl:"components/match.html"}).state("app.mail",{url:"/mail",templateUrl:"components/mail.html"}).state("app.vote",{url:"/vote",templateUrl:"components/voting.html"}).state("appSimp",{abstract:!0,templateUrl:"components/layout/simp.html"}).state("appSimp.login",{url:"/login",templateUrl:"components/login.html"}).state("appSimp.register",{url:"/register",templateUrl:"components/register.html"}).state("appSimp.notfound",{url:"/404",templateUrl:"components/alt/404.html"}).state("appSimp.err",{url:"/500",templateUrl:"components/alt/500.html"}),n.interceptors.push(["$q",function(t){return{request:function(e){return e},requestError:function(e){return t.reject(e)},response:function(t,o){return t&&t.data&&"refresh"==t.data&&e.emit("requestRefresh",{id:e.id}),t},responseError:function(e){return t.reject(e)}}}])}]).directive("changeReFocus",[function(){return{restrict:"EA",scope:{changeFn:"&"},link:function(e,t,o){t.bind("change",function(o){e.changeFn().then(e=>{t[0].focus()})})}}}]).directive("fileread",["$log",function(e){return{scope:{fileread:"="},link:function(t,o,s){o.bind("change",function(o){const s=new FileReader,r=o.target.files[0],a=r.name;return e.debug("UPLOADING FILE",r),s.onload=function(o){let s=o.target.result;e.debug("URI before optional resize",s,s.length),t.$parent.needsResize?n(t,s,t.$parent.needsResize,t.$parent.needsResize,a):(e.debug("APPLYING file to $parent",t.$parent),t.$apply(function(){t.$parent&&t.$parent.$parent&&t.$parent.$parent.avas?(t.$parent.$parent.loadingFile=!1,t.$parent.$parent.fileName="Loaded:"+a,t.$parent.$parent.fileread=s):(t.$parent.loadingFile=!1,t.$parent.fileName="Loaded:"+a,t.$parent.fileread=s),t.$parent.saveDataURI&&"function"==typeof t.$parent.saveDataURI&&t.$parent.saveDataURI(dataURI)}))},r?r.size>25e5?(bulmabox.alert("File too Large",`Your file ${r.name} is larger than 2.5MB. Please upload a smaller file!`),!1):void s.readAsDataURL(r):(t.$apply(function(){t.fileread="",t.$parent.fileName=!1,t.$parent.loadingFile=!1}),!1)})}}}]).directive("waitForData",[function(){return{restrict:"A",link:function(e,t,o){e.wfdArr=o.waitForData.split(",").map(e=>e.trim());const s=document.createElement("div");s.id="bg-wait",s.innerHTML='Loading <div class="loady-spin"></div>',document.querySelector("body").append(s),document.querySelector("#full-win").style.filter="blur(9px)";let n=e.wfdArr.map(function(t){const o={fn:null,name:t,data:null};return o.fn=e.$watch(t,function(t,s){o.data=t,n.filter(e=>!!e.data).length>=e.wfdArr.length&&(n=[],document.querySelector("body").removeChild(document.querySelector("#bg-wait")),document.querySelector("#full-win").style.filter="none")}),o})}}}]),String.prototype.titleCase=function(){return this.split(/\s/).map(e=>e.slice(0,1).toUpperCase()+e.slice(1).toLowerCase()).join(" ")};const n=(e,t,o,s,n)=>{const r=document.createElement("img");r.onload=function(){const t=document.createElement("canvas"),r=t.getContext("2d");t.width=o,t.height=s,r.drawImage(this,0,0,o,s);const a=t.toDataURL();e.$apply(function(){e.$parent.loadingFile=!1,e.$parent.fileName="Loaded:"+n,e.fileread=a,e.$parent.saveDataURI&&"function"==typeof e.$parent.saveDataURI&&e.$parent.saveDataURI(a)})},r.src=t};function r(e){return{restrict:"A",priority:101,link:function(t,o,s){e(function(){t.$evalAsync(s.postrenderAction)},0)}}}t.directive("postrenderAction",r),t.controller("dash-cont",["$scope","$http","$q","userFact","$log","$sce",(t,o,s,n,r,i)=>{t.refUsr=t.$parent.refUsr,t.refUsr(),t.updateTopics=()=>{o.put("/user/interests",t.$parent.user.interests).then(e=>{})},t.removeSkill=e=>{o.delete("/user/interests?t="+e).then(e=>{})},t.debounceTimer=null,t.saveDebounce=e=>{t.debounceTimer&&clearTimeout(t.debounceTimer),t.debounceTimer=setTimeout(function(){t.saveGeneral()},500)},t.saveGeneral=()=>{const e=t.user.displayName;o.put("/user/profile",t.user).catch(t=>{"dupDisplay"==t.data&&bulmabox.alert("Duplicate Name",`Sorry, but the name ${e} is already in use. Please use another name.`)})},t.loadFile=e=>{r.debug("loadFile sez",e),t.loadingFile=!0;new FileReader},t.loadingFile=!1,t.fileName=null,t.topicToAdd="",t.needsResize=200,t.saveDataURI=e=>{t.user.avatar=e,t.saveGeneral()},t.filterMe=e=>{const o=e.toLowerCase();if(!t.topicObjs)return[];return t.topicObjs.filter(e=>e.value.indexOf(o)>-1)},t.hazTopic=e=>t.topicObjsAll&&t.topicObjsAll.length&&e&&t.topicObjsAll.find(t=>t.display==e),t.newTopic={title:null,desc:null,show:!1},t.refTopObjs=e=>{o.get("/topic/topic",{headers:{"Cache-Control":"no-cache"}}).then(o=>{t.topicObjsAll=o.data.map(e=>({value:e.title.toLowerCase(),display:e.title,desc:e.desc})),r.debug("All Topic Objs now:",t.topicObjsAll),t.topicObjs=angular.copy(t.topicObjsAll),e&&e()})},t.refTopObjs(),t.toggleNewTopicDia=()=>{if(!t.newTopic.show){t.newTopic.title=t.topicToAdd;const e=document.querySelector(".md-menu-showing"),o=document.querySelector("md-virtual-repeat-container");e&&e.classList.remove("md-menu-showing"),o&&o.classList.add("ng-hide")}t.newTopic.show=!t.newTopic.show},t.addNewTopic=()=>{o.post("/topic/topic",t.newTopic).then(o=>{e.emit(),t.toggleNewTopicDia()}).catch(e=>{"duplicate"==e.data&&bulmabox.alert("Duplicate Topic","Hey! That topic already exists!")})},t.addInt={title:null,show:!1,lvl:0,newDesc:null,canTeach:!1},t.addIntDial=e=>{t.refTopObjs(function(){t.addInt={title:e||null,show:!0,lvl:0,newDesc:null,canTeach:!1}})},t.deFocusSkillSearch=()=>{setTimeout(function(){const e=document.getElementFromPoint(t.mouse.x,t.mouse.y);console.log("new focus",e)},1)},t.mouse={x:0,y:0},document.querySelector("#skill-search").addEventListener("mousemove",e=>{t.mouse.x=e.clientX,t.mouse.y=e.clientY}),document.querySelector("#skill-search input").addEventListener("keyup",e=>{"Escape"==e.key&&(t.clearSkillSearchBox(),t.$apply())}),t.skillSearchFilter=()=>t.skillSearch?t.topicObjs.filter(e=>e.value.includes(t.skillSearch.toLowerCase())).map(e=>(e.displayHL=i.trustAsHtml(e.display.replace(new RegExp(t.skillSearch,"gi"),function(e,t,o){return"<strong>"+e+"</strong>"})),e)):[],t.clearSkillSearchBox=()=>{console.log("clearing Skillbox"),t.skillSearch=null},t.pickSkill=(e,o)=>{o.stopPropagation(),console.log("user wants to pick skill",e),t.skillSearch=null,t.selectedTopic=e,t.$digest()},e.on("topicRef",function(e){bulmabox.confirm("Topic Refresh","One or more topics have been update. Would you like to refresh the page to make these new topics available?",e=>{if(e)return t.refTopObjs()})});let l=!1;t.saveSkills=()=>{if(!t.topicToAdd)return bulmabox.alert("No Skill Name","Please enter a skill name!");const e=t.$parent.user.interests;if(e.find(e=>e.title==t.topicToAdd))return t.topicToAdd="",bulmabox.alert("Duplicate Skill","You've already added that skill!");t.hazTopic(t.topicToAdd)?(e.push({title:t.topicToAdd,lvl:t.addInt.lvl,canTeach:!!t.addInt.canTeach}),t.topicToAdd="",o.put("/user/interests",e).then(e=>{t.addInt={title:null,show:!1,lvl:0,canTeach:!1}})):(l=!0,o.post("/topic/topic",{title:t.topicToAdd,desc:t.addInt.newDesc}).then(e=>{t.refTopObjs(t.saveSkills)}))},t.modProj={show:!1,proj:null,editMode:!1},t.modProjDial=e=>{e?t.modProj.editMode=!0:(e={name:null,description:null,position:null},t.modProj.editMode=!1),t.modProj.proj=e,t.modProj.show=!0},t.saveProjs=e=>{const s=angular.copy(t.$parent.user.projects);r.debug("Saving projects. List is",s,"modProj",t.modProj),t.modProj.editMode||s.push(angular.copy(t.modProj.proj));const n=a(s,"name");if(n)return bulmabox.alert("Duplicate Project Name",`The name ${n} is already being used! Please pick another one for this project.`);o.put("/user/projs",s).then(e=>{t.modProj={show:!1,proj:null,editMode:!1}})},t.projView={proj:null,show:!1},t.viewEditProj=e=>{t.projView.proj=e,t.show=!0},t.deleteProj=e=>{bulmabox.confirm("Remove Project",`Are you sure you wish to remove the project ${e}?`,t=>{t&&o.delete("/user/projs",{data:{name:e},headers:{"Content-Type":"application/json;charset=utf-8"}}).then(e=>{})})},t.countDups=a,t.messageTeacher=e=>{bulmabox.confirm("Message Mentor","Do you wish to send a message to this mentor that you wish to discuss the lesson?",function(t){if(!t||null==t)return!1;o.post("/user/reqDiscussLesson?id=",e).then(e=>{bulmabox.alert("Message Sent","Your mentor has been notified that you wish to discuss this lesson.")})})},t.reqEndLesson=e=>{bulmabox.confirm("Request Lesson End","Do you wish to notify this mentor that you wish to end the lesson?",function(t){if(!t||null==t)return!1;o.post("/user/reqEndLesson?id=",e).then(e=>{bulmabox.alert("Lesson End Requested","Your mentor has been notified that you wish to end this lesson. <br>Please note that it is still up to them to end the lesson.")})})},t.reportLesson=e=>{bulmabox.confirm("Report Lesson","Are you sure you wish to report this lesson to the moderator team? <br>Please note that abuse of the report feature in any context is grounds for account termination.",function(t){if(!t||null==t)return!1;o.post("/usr/repLesson",e).then(e=>{bulmabox.alert("Lesson Reported","This lesson has been reported to the moderator team. In addition, the lesson has automatically been stopped, and a message has been sent to the lesson's mentor.")})})},t.getLessons=()=>{o.get("/user/activeLessons").then(e=>{r.debug("ACTIVE LESSONS",e),t.activeLessons=e&&e.data||[]})},t.getLessons(),t.writeReview=async function(e){t.reviewPage.msg="",t.reviewPage.stars=5,t.reviewPage.currStars=5,t.reviewPage.teacher={user:e.user,displayName:e.displayName||null},t.reviewPage.hideName=!1;const s=await o.get(`/user/review?tch=${e.displayName||e.user}`);r.debug("OLD REVIEW",s),s&&(t.reviewPage.msg=s.data.rateText,t.reviewPage.stars=s.data.rateNum,t.reviewPage.currStars=s.data.rateNum,t.reviewPage.hideName=s.data.hideName),t.reviewPage.starsArr=[0,1,2,3,4],t.reviewPage.show=!0,t.$apply()},t.overStars=!1,t.toggleOverStars=(e,o)=>{if(!1===e)return t.overStars=!1,!1;t.overStars=o},t.isActiveStar=(e,o,s)=>t.overStars||!1!==t.overStars?!1!==t.overStars&&t.overStars+1>e:e<t.reviewPage.stars,t.reviewPage={show:!1,stars:5,currStars:5,teacher:{user:"Nothere",displayName:"DoesntExist"},msg:null},t.submitReview=()=>{const e={rateNum:t.reviewPage.stars,hideName:!!t.reviewPage.hideName,rateText:t.reviewPage.msg,tch:{user:t.reviewPage.teacher.user,displayName:t.reviewPage.teacher.displayName}};r.debug("WOULD SUBMIT",e),o.put("/user/review",e).then(e=>{t.reviewPage={show:!1,stars:5,currStars:5,teacher:{user:"Nothere",displayName:"DoesntExist"},msg:null}})}}]);const a=(e,t)=>{if(!e||!e.length)return!1;const o={};for(let s=0;s<e.length;s++){if(o[e[s][t]])return o[e[s][t]];o[e[s][t]]=1}return!1};t.controller("log-cont",["$scope","$http","$state","$q","userFact","$log",function(t,o,s,n,r,a){t.noWarn=!1,t.nameOkay=!0,delete localStorage.geoUsr,t.acceptNoSecure=()=>{localStorage.CMMNoSecure=!0,t.cmmNoSecure=!0},t.cmmNoSecure=!!localStorage.CMMNoSecure,t.checkTimer=!1,t.goReg=()=>{s.go("appSimp.register")},t.goLog=()=>{s.go("appSimp.login")},t.googLog=()=>{window.location.href="./user/google"},t.forgot=()=>{t.user?r.forgot({user:t.user}).then(function(e){a.debug("forgot route response",e),"err"==e.data?bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Forgot Password Error',"It looks like that account either doesn't exist, or doesn't have an email registered with it! Contact a mod for further help."):bulmabox.alert("Forgot Password","Check your email! If your username is registered, you should recieve an email from us with a password reset link.")}):bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Forgot Password',"To recieve a password reset email, please enter your username!")},t.signin=()=>{a.debug("trying to login with",t.user,t.pwd),r.login({user:t.user,pass:t.pwd}).then(o=>{a.debug(o),"authErr"!=o.data&&o.data?"banned"==o.data?bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Banned',"You've been banned!"):(a.debug("LOGIN RESPONSE",o.data),e.emit("chatMsg",{msg:`${t.user} logged in!`}),localStorage.geoUsr=JSON.stringify(o.data.usr),o.data.news&&bulmabox.alert("Updates/News",`Since you last logged in, the following updates have been implemented:<br><ul style='list-style:disc;'><li>${o.data.news.join("</li><li>")}</li></ul>`),s.go("app.dash")):bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Incorrect Login',"Either your username or password (or both!) are incorrect.")}).catch(e=>{if("banned"==e.data.status)return bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Banned',`You've been banned by moderator ${e.data.usr}!`);bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Error',"There's been some sort of error logging in. This is <i>probably</i> not an issue with your credentials. Blame the devs!"),a.debug(e)})},t.checkUser=()=>{t.checkTimer&&clearTimeout(t.checkTimer),t.checkTimer=setTimeout(function(){r.nameCheck(t.user).then(e=>{t.nameOkay=e.data})},500)},t.emailBad=!1,t.checkEmail=()=>{t.emailBad=t.regForm.email&&t.regForm.email.$viewValue.length&&!t.regForm.email.$valid},t.pwdNoDup=!1,t.checkPwdDup=()=>{t.pwdNoDup=!t.pwd||!t.pwdDup||t.pwdDup!==t.pwd},t.pwdStrStars=[0,1,2,3,4],t.badPwds=["password","pass","1234","123","admin","abc","abcd","pwd"],t.pwdStr={recs:[],score:0,maxScore:5,show:!1},t.checkPwdStr=()=>{if(!t.pwd)return!1;const e=[{desc:"Using at least one uppercase letter",reg:"[A-Z]"},{desc:"Using at least one lowercase letter",reg:"[a-z]"},{desc:"Using at least one number",reg:"[0-9]"},{desc:'Using at least one "special" character (@, !, $, etc.)',reg:"[@!$^_*&]"},{desc:"Using at least 12 characters",reg:"[\\w]{12}"},{desc:"Not using a bad password",negate:!0,reg:["password","pass","1234","123","admin","abc","abcd","pwd"].map(e=>`(${e})`).join("|")}],o=e.filter(e=>{const o=new RegExp(e.reg);return e.negate?!!o.test(t.pwd):!o.test(t.pwd)});t.pwdStr={recs:o,score:e.length-o.length,maxScore:5,show:t.pwdStr.show}},t.register=()=>{t.pwd&&t.pwdDup&&t.user?t.pwd!=t.pwdDup?(a.debug("derp"),bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Password Mismatch',"Your passwords don't match, or are missing!")):!t.email||t.emailBad?bulmabox.confirm('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Send Without Email?',"You've either not included an email, or the format you're using doesn't seem to match any we know. <br>While you <i>can</i> register without a valid email, it'll be much more difficult to recover your account if you forget your password!<br>Register anyway?",function(e){if(!e||null==e)return!1;r.newUser({user:t.user,pass:t.pwd,email:t.email}).then(e=>{r.login({user:t.user,pass:t.pwd}).then(()=>{s.go("app.dash")}).catch(e=>{"duplicate"==e.data&&bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;User Already Exists',"That account already exists. Are you sure you didn't mean to log in?")})}).catch(e=>{"duplicate"==e.data&&bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;User Already Exists',"That account already exists. Are you sure you didn't mean to log in?")})}):(a.debug("running register with user",t.user,"and pwd",t.pwd),r.newUser({user:t.user,pass:t.pwd,email:t.email}).then(e=>{r.login({user:t.user,pass:t.pwd}).then(()=>{s.go("app.dash")})}).catch(e=>{"duplicate"==e.data&&bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;User Already Exists',"That account already exists. Are you sure you didn't mean to log in?")})):bulmabox.alert('<i class="fa fa-exclamation-triangle is-size-3"></i>&nbsp;Missing Information',"Please enter a username, and a password (twice).")}}]),t.controller("mail-cont",["$scope","$http","$q","$log","userFact",(e,t,o,s,n)=>{s.debug("Mailbox ctrl registered"),e.outMode=!1,e.mailView={title:"No Message Selected",htmlMsg:"Not sure how you got here, but there is not message here!",mdMsg:"",show:!1,toMode:!1,date:12345,other:!1},e.viewMsg=t=>{s.debug("incoming msg object is",t),e.mailView.title=t.to?`Message to ${e.getUserList(t.to)}`:`Message from ${t.from.displayName||t.from.user}`,e.mailView.htmlMsg=t.htmlMsg,e.mailView.isConMsg=!!t.isConMsg,e.mailView.date=t.date,e.mailView.toMode=!!t.to,e.mailView.other=t.to||t.from,e.mailView.topics=t.topics,e.mailView._id=t._id,e.mailView.msgId=t.msgId||0,e.mailView.show=!0,e.mailView.isRep=!!t.isRep},e.newMsg={to:[],htmlMsg:"",isReply:!1,show:!1},e.reportMsg=e=>{bulmabox.confirm("Report Message",`Are you sure you wish to report this message from ${e.other} to the moderator team?`,o=>{o&&null!=o&&t.post("/user/repMsg",e).then(e=>{bulmabox.alert("Reported!","A notification has been sent to the moderator team!")})})},e.startTeach=()=>{t.put("/user/teach",e.mailView).then(t=>{e.mailView.show=!1})},e.explStartTeach=()=>{bulmabox.alert("Start Teaching","Clicking the \"Teach\" button will allow the student to leave reviews for you. <br>While there's nothing explicitly preventing you from connecting on your own, we'd still recommend you click this button, as well-written reviews are always helpful!")},e.replyMsg=t=>{s.debug("attempting to setup reply to msg",t),e.newMsg.show=!0,e.newMsg.to=[t.from||t.other],e.newMsg.isReply=t,e.mailView={title:"No Message Selected",htmlMsg:"",mdMsg:"",rawMsg:"",show:!1,toMode:!1,date:12345,other:!1},s.debug("newMsg now",e.newMsg)},e.cancelSend=()=>{e.newMsg.mdMsg&&e.newMsg.mdMsg.length?bulmabox.confirm("Discard Message","Are you sure you wish to discard this message?",t=>{t&&null!=t&&(e.newMsg={to:[],htmlMsg:"",isReply:!1,show:!1},e.$digest())}):e.newMsg={to:[],htmlMsg:"",isReply:!1,show:!1}},e.makeNewMsg=()=>{e.newMsg.show=!0,e.newMsg.to=[],e.newMsg.isReply=!1},e.sendMsg=()=>{if(!e.newMsg.to.length||!e.newMsg.mdMsg||!e.newMsg.mdMsg.length)return bulmabox.alert("Missing Information","Please provide at least one user and something to say!");const t=new showdown.Converter;e.newMsg.htmlMsg=t.makeHtml(e.newMsg.mdMsg),e.newMsg.isReply&&(e.newMsg.htmlMsg=`${e.newMsg.isReply.htmlMsg}<hr>${e.newMsg.htmlMsg}`,e.newMsg.mdMsg=`${e.newMsg.isReply.mdMsg}\n---\n${e.newMsg.mdMsg}`),n.sendMsg({to:e.newMsg.to,htmlMsg:e.newMsg.htmlMsg,mdMsg:e.newMsg.mdMsg}).then(t=>{e.newMsg={to:[],htmlMsg:"",isReply:!1,show:!1},bulmabox.alert("Message Sent!","Your message is on its way!")})},e.newMsgAdd=t=>{if(!e.newMsg.show)return!1;if("enter"!==t.key.toLowerCase()||e.newMsg.to.includes(e.newMsg.possUsr)){if("enter"===t.key.toLowerCase()){(function(){let t=250;const o=document.querySelector(`.user-to-tag[data-name='${e.newMsg.possUsr}']`),s=setInterval(function(){if((t-=10)<0)return clearInterval(s);const e=`hsl(0,${~~(100*t/250)}%,${~~((250-t)/250*46)+50}%)`;o.style.background=e},10)})();e.newMsg.possUsr=""}}else e.newMsg.to.push(e.newMsg.possUsr),e.newMsg.possUsr=""},e.newMsgRem=t=>{if(!e.newMsg.show)return!1;e.newMsg.to=e.newMsg.to.filter(e=>e!=t)},e.delMsg=e=>{bulmabox.confirm("Delete Message","Are you sure you wish to delete this message? This is <i>not</i> reversable!",o=>{if(o&&null!=o){const o=e.from?`/user/delMsg?msgId=${e.msgId}`:`/user/delMyMsg?msgId=${e.msgId}`;t.get(o).then(e=>{})}})},e.getUserList=e=>e.map(e=>e.displayName||e.user).join(", ")}]),String.prototype.capMe=function(){return this.slice(0,1).toUpperCase()+this.slice(1)},t.controller("main-cont",["$scope","$http","$state","userFact","$log",function(t,o,s,n,r){r.debug("main controller registered!"),t.user=null,t.refUsr=()=>{n.getUser().then(e=>{t.user=e.data})},e.on("refreshById",e=>{n.getUser().then(e=>{t.user=e.data,t.$apply()})}),t.refUsr(),e.on("refresh",e=>{t.user&&e.user==t.user.user&&t.refUsr()}),t.isActive=!0,t.pokeTimer=null,t.faceOpen=!1;window.addEventListener("focus",function(e){t.isActive=!0,t.pokeTimer&&clearInterval(t.pokeTimer),document.title="CodeMentorMatch"}),window.addEventListener("blur",function(e){t.isActive=!1});const a=["😐","😮"];e.on("chatMsgOut",e=>{e.msg.includes("@"+t.user.user)&&e.user!=t.user.user&&!t.isActive&&(t.pokeTimer=setInterval(function(){t.faceOpen=!t.faceOpen;let e=t.faceOpen?0:1;document.title=a[e]+" CodeMentorMatch"},250))}),e.on("reqHeartBeat",function(o){t.alsoOnline=o.filter(e=>!t.user||!t.user.user||t.user.user!=e.name).map(e=>e.name),t.user&&t.user.user&&s.current.name.includes("app.")&&e.emit("hbResp",{name:t.user.user})}),e.on("disco",e=>{e.on?t.col="div:nth-child(even){animation:huehue 4s linear 2s infinite;}div:nth-child(odd){animation:huehue 4s linear 0s infinite;}":t.col="div:nth-child(even){animation:none;}div:nth-child(odd){animation:none}",t.$apply()}),t.explMd=()=>{bulmabox.alert("Markdown",'<div class=\'is-size-2\'>Markdown</div>\n        <hr>\n        <div class=\'is-size-5\'>What It Is</div>\n        <p>Markdown is a specialized way of formatting text, used by sites like Reddit, Stack Overflow, and apps like Discord.</p>\n        <hr>\n        <div class=\'is-size-5\'>What It Does</div>\n        <p>It allows you to format text with stuff like <strong>bold</strong> or <em>italics</em> without the use of fancy, complex word processors.</p>\n        <hr>\n        <div class=\'is-size-4\'>Commands:</div>\n        <table class="table table-striped table-bordered">\n        <thead>\n        <tr>\n        <th style="text-align:left"><strong>Markdown</strong></th>\n        <th style="text-align:left"><strong>Result</strong></th>\n        </tr>\n        </thead>\n        <tbody>\n        <tr>\n        <td style="text-align:left">#text, ##text,…</td>\n        <td style="text-align:left">header (big text). More \'#\'s means smaller headers</td>\n        </tr>\n        <tr>\n        <td style="text-align:left">*text*, _text_</td>\n        <td style="text-align:left"><em>italic text</em></td>\n        </tr>\n        <tr>\n        <td style="text-align:left">**text**,__text,__</td>\n        <td style="text-align:left"><strong>bold text</strong></td>\n        </tr>\n        <tr>\n        <td style="text-align:left">~~text~~</td>\n        <td style="text-align:left"><s>strikethrough text</s> woops!</td>\n        </tr>\n        <tr>\n        <td style="text-align:left">[link text](link url)</td>\n        <td style="text-align:left"><a href="https://www.google.com">links</a></td>\n        </tr>\n        <tr>\n        <td style="text-align:left">- item</td>\n        <td style="text-align:left">- item in a bullet list</td>\n        </tr>\n        <tr>\n        <td style="text-align:left">1. item</td>\n        <td style="text-align:left">1. Item in a numbered list</td>\n        </tr>\n        <tr>\n        <td style="text-align:left">![alt text](url “hover text”)</td>\n        <td style="text-align:left"><img src="https://github.com/adam-p/markdown-here/raw/master/src/common/images/icon48.png" alt="alt text" title="hover text"> (The alt text is displayed if the browser can’t load the image)</td>\n        </tr>\n        <tr>\n        <td style="text-align:left">`text`</td>\n        <td style="text-align:left"><code>look ma, ima programmer!</code></td>\n        </tr>\n        </tbody>\n        </table>')},t.sc="",t.seekrit={code:["arrowup","arrowup","arrowdown","arrowdown","arrowleft","arrowright","arrowleft","arrowright","b","a","Space"],on:!1,asking:!1,hist:[],corrNum:0,whichFont:0},t.fontOpts=["aurebesh","tengwar quenya-1","klingon font","hieroglyphic","dovahkiin","Skyrim_Daedra"];const i=`./img/bgs/bg${Math.ceil(3*Math.random())}.jpg`;t.bgFull=`background:linear-gradient(rgba(0,0,0,.4),rgba(0,0,0,.4)),url('${i}'); background-size:cover; background-attachment:fixed; will-change:transform; background-repeat:no-repeat`,document.addEventListener("scroll",function(e){setTimeout(function(){const e=.5+.45*window.scrollY/window.outerHeight;t.bgFull=`background:linear-gradient(rgba(0,0,0,${e}),rgba(0,0,0,${e})),url('${i}'); background-size:cover; background-attachment:fixed;will-change:transform; background-repeat:no-repeat`,t.$apply()},100)}),document.querySelector("body").addEventListener("keyup",function(e){if(e.preventDefault(),t.seekrit.asking)return!1;const o=t.seekrit.code[t.seekrit.corrNum];if(e.key.toLowerCase()!=o&&e.code!=o)return!1;t.seekrit.corrNum+1===t.seekrit.code.length?(t.seekrit.asking=!0,bulmabox.confirm("Toggle Secret Mode","Are you sure you wanna toggle the Secret Mode?",function(e){if(e&&null!==e){if(t.seekrit.on=!t.seekrit.on,t.seekrit.on){let e=t.fontOpts[t.seekrit.whichFont];t.seekrit.whichFont++,t.seekrit.whichFont>=t.fontOpts.length&&(t.seekrit.whichFont=0),t.sc=`*:not(.fa){font-family:${e},arial!important;}`}else t.sc="";t.$digest()}else t.sc="",t.$digest();t.seekrit.asking=!1,t.seekrit.corrNum=0})):t.seekrit.corrNum++})}]).filter("numToDate",function(){return function(e){if(isNaN(e))return"Invalid date!";const t=new Date(e);return`${t.toLocaleDateString()} ${t.getHours()%12}:${t.getMinutes().toString().length<2?"0"+t.getMinutes():t.getMinutes()} ${t.getHours()<13?"AM":"PM"}`}}),t.controller("match-cont",["$scope","$http","$q","$log",function(t,o,s,n){n.debug("matcher ctrl registered"),t.regetTopics=()=>{o.get("/topic/topic").then(e=>{t.topicObjsAll=e.data.map(e=>({value:e.title.toLowerCase(),display:e.title,desc:e.desc})),t.topicObjs=angular.copy(t.topicObjsAll),t.topicObjs.push({value:"addSkill",display:"---- + Add a skill ----",desc:"Add a new skill!"})})},e.on("topicRef",function(e){bulmabox.confirm("Topic Refresh","One or more topics have been update. Would you like to refresh the page to make these new topics available?",e=>{if(e)return window.location.reload(!0)})}),t.regetTopics(),t.clearSearch=()=>{t.skillSearch="",t.showNSR=!1,t.$digest()},t.showNSR=!1,new autoComplete({data:{src:async()=>{t.skillSearch;const e=await fetch("/topic/topic"),o=await e.json();return console.log("data",o),o},key:["title"],cache:!1},sort:(e,t)=>e.match-t.match,placeHolder:"Pick a skill",selector:"#autoComplete",threshold:0,debounce:0,searchEngine:"strict",highlight:!0,resultsList:{render:!0,container:e=>{e.setAttribute("id","skill-list")},destination:document.querySelector("#autoComplete"),position:"afterend",element:"ul"},resultItem:{content:(e,t)=>{console.log("in resultItem",e,t),t.innerHTML=e.match},element:"li"},noResults:()=>{console.log("TRIGGERED NSR CALLBACK",t.skillSearch,t.showNSR),t.showNSR=!0,console.log("SHOWNSR NOW",t.showNSR),t.$apply()},onSelection:e=>{t.selectedTopic=e.selection.value,t.$digest(),document.querySelector("#autoComplete").value=""}}),t.pickedItem=null,t.newTopic={title:null,desc:null,show:!1},t.toggleNewTopicDia=()=>{document.dispatchEvent(new Event("click")),t.newTopic.title=t.newTopic.show?null:t.topicToAdd;const e=document.querySelector(".md-standard-list-container.md-autocomplete-suggestions-container");t.newTopic.show||(e.className+=" ng-hide"),t.newTopic.show=!t.newTopic.show,!t.newTopic.show&&r&&(r=!1,t.regetTopics())};let r=!1;e.on("topicUpdate",e=>{t.newTopic.show?r=!0:t.regetTopics()}),t.AddNewSkill=e=>{e.preventDefault(),console.log("user wants to add skill called ",t.skillSearch),t.clearSearch()},t.addNewTopic=()=>{o.post("/topic/topic",t.newTopic).then(e=>{t.toggleNewTopicDia()}).catch(e=>{"duplicate"==e.data&&bulmabox.alert("Duplicate Topic","Hey! That topic already exists!")})},t.pickedTopics=[],t.addTopicBtn=()=>"Nothing to see here!",t.changeTopList=()=>{const e=t.pickedTopics.map(e=>e.value.toLowerCase());t.topicObjs=t.topicObjsAll.filter(t=>!e.includes(t.value)),o.post("/user/topicSearch",t.pickedTopics).then(e=>{t.availTeachs=e.data})},t.addSearchTopic=e=>{if(n.debug("topic",e),t.pickedTopics.map(e=>e.value.toLowerCase()).includes(e.value))return t.topicToAdd="",bulmabox.alert("Already Added","You've already added this topic!");const o=angular.copy(e);o.min=1,t.pickedTopics.push(o),t.changeTopList(),t.topicToAdd=""},t.removeTopic=e=>{t.pickedTopics=t.pickedTopics.filter(t=>t.value!=e.value),t.changeTopList()},t.mentCon={topics:[],user:null,show:!1,tz:{from:null,to:null,offset:(new Date).getTimezoneOffset()},plusMdMsg:null,plusHtmlMsg:null},t.times=new Array(24).fill(1).map((e,t)=>{let o=null;return{label:o=t<12?t+1+":00 AM":t-11+":00 PM",num:t}}),t.lvls=new Array(10).fill(1).map((e,t)=>({lbl:t>1?t:"None",num:t})),t.conv=new showdown.Converter,t.doConnect=e=>{t.mentCon.user=e.user,t.mentCon.displayName=e.displayName||null,t.mentCon.topics=t.pickedTopics.map(e=>e.value),t.mentCon.plusMdMsg=null,t.mentCon.plusHtmlMsg=null,t.mentCon.tz={from:null,to:null},t.mentCon.show=!0,t.mentCon.topics=t.pickedTopics.map(e=>e.display)},t.sendConnectMsg=e=>{t.mentCon.plusHtmlMsg=t.conv.makeHtml(t.mentCon.plusMdMsg),o.put("/user/connect",t.mentCon).then(e=>{t.mentCon.show=!1,bulmabox.alert("Connect Request Sent",`User ${t.mentCon.displayName||t.mentCon.user} has been notified that you'd like them as a mentor!`)})},t.reqLess=()=>{n.debug("User",t.$parent.user,"wants lesosn on topics",t.pickedTopics);const e=t.pickedTopics.map(e=>e.display);n.debug("Final LRO:",e),o.post("/user/lessonReq",e).then(e=>{bulmabox.alert("Lesson Request Sent",'Your lesson request has been sent! You can head on over to the "View Requested Lessons" tab if you wanna view or delete it.')}).catch(e=>{bulmabox.alert("Duplicate Lesson Request","You've already requested a lesson for these skills!")})},t.answerRl=e=>{const s=e.topics.map(e=>{const o=t.$parent.user.interests.find(t=>t.title==e.title);return o?o.lvl<=e.lvl?`<li>${e.title}  <div class='tag is-danger' title='Your level in this skill is less than or equal to the student's level!'><div class='fa fa-exclamation-triangle'></div></div></li>`:`<li>${e.title}</li>`:`<li>${e.title}  <div class='tag is-warning' title='This skill is not in your list of personally known skills!'><div class='fa fa-exclamation-triangle'></div></div></li>`}).join("");bulmabox.confirm("Teach Lesson",`Are you sure you wish to teach a lesson on the following topics?\n        <ul class="contents">\n        ${s}\n        </ul>`,s=>{s&&null!=s&&o.post("/user/teachLessonReq",{id:e._id}).then(e=>{bulmabox.alert("Teach Offer Sent","This student has been notified that you wish to teach this lesson. You'll be notified when they respond to your request."),t.regetReqLsns()})})},t.noAnswerRl=e=>{bulmabox.confirm("Cancel Teaching Offer",`Are you sure you wish to rescind your offer to teach these skills to ${e.displayName||e.user}?`,s=>{s&&null!=s&&o.post("/user/teachLessonReq",{id:e._id}).then(e=>{t.regetReqLsns()})})},t.deleteRl=e=>{n.debug("WANNA DELETE LESSON",e),bulmabox.confirm("Delete Lesson Request","Are you sure you wish to delete this lesson request?<br/>You'll need to make a new one if you wanna learn these skills!",s=>{s&&null!=s&&o.delete("/user/lessonReq?id="+e._id).then(e=>{t.regetReqLsns()})})},t.acceptMentor=(e,s)=>{bulmabox.confirm("Accept Mentor","Are you sure you wish to accept this mentor? This will remove the lesson from the list of Requested Lessons and send a message to both of you to connect.",n=>{n&&null!=n&&o.post("/user/acceptLesson",{id:s._id,teacher:e}).then(e=>{t.regetReqLsns()})})},t.regetReqLsns=()=>{o.get("/user/lessonReq").then(e=>{t.requestedLessons=e.data})},t.tchrInfo={teacher:null,show:!1},t.showTchrInfo=e=>{t.tchrInfo.tchr=e,t.tchrInfo.show=!0},t.totalStars=[0,1,2,3,4],e.on("refReqLs",e=>{t.regetReqLsns()})}]),t.controller("mentor-cont",["$scope","$http","$q","userFact","$log",(e,t,o,s,n)=>{n.debug("mentor ctrl registered"),e.totalStars=[0,1,2,3,4],e.messageUser=e=>{n.debug("sending message to",e.user),t.post("/user/LCmsgStudent",e).then(e=>{bulmabox.alert("Message Sent","Your student {{l.user.displayName||l.user.user}} has been notified that you wish to speak with them regarding this lesson.")})},e.toggleActive=e=>{const o=e.active?"Deactivate Lesson":"Re-activate Lesson",s=e.active?"Are you sure you wish to deactivate (end) this lesson?":"Are you sure you wish to re-activate (resume) this lesson?";bulmabox.confirm(o,s,o=>{o&&t.get("/user/LCtoggleActive?id="+e._id).then(e=>{})})},e.toggleHide=e=>{const o=e.active?"Hide Lesson":"Un-Hide Lesson",s=e.active?"Are you sure you wish to hide this lesson?<br>Hiding a lesson will prevent it from showing up on your Teacher Review panel.":"Are you sure you wish to un-hide this lesson? <br>Un-hiding a lesson will make it visible again on your Teacher Review panel.";bulmabox.confirm(o,s,o=>{o&&t.get("/user/LCtoggleHide?id="+e._id).then(e=>{})})}}]),t.controller("mod-cont",["$scope","$http","$state","$log","userFact",function(e,t,o,s,n){setInterval(function(){e.$parent&&e.$parent.user&&e.$parent.user.mod||"app.mod"!=o.current.name||(console.log("User not mod"),o.go("app.dash"))},1),console.log("current state",o.current),e.getAllUsers=()=>{t.get("/user/users").then(t=>{e.allUsrs=t&&t.data}).catch(e=>{if(e.data&&"noUsrs"==e.data)return bulmabox.alert("No Users!","Something's gone wrong, and we can't find any users.")})},e.getAllUsers(),e.toggleBan=o=>{if(o.user===e.$parent.user.user)return bulmabox.alert("Cannot Ban Self","You can't ban yourself!");const s=o.isBanned?"Unban User":"Ban User",n=o.isBanned?`Are you sure you wish to unban ${o.displayName||o.user}? This will restore their access to CodeMentorMatch.`:`Are you sure you wish to ban ${o.displayName||o.user}? This will revoke their access to CodeMentorMatch.`;bulmabox.alert(s,n,s=>{s&&t.put("/user/toggleBan",{user:o.user}).then(t=>{e.getAllUsers()})})},e.showMsgDial=t=>{e.newMsg={user:t.user,displayName:t.displayName,msg:null,show:!0}},e.cancelMsg=()=>{e.newMsg={user:null,displayName:null,msg:null,show:!1}},e.newMsg={user:null,displayName:null,mdMsg:null,show:!1},e.sendMsg=()=>{if(!e.newMsg.mdMsg||e.newMsg.mdMsg.trim().length)return bulmabox.alert("No Message","Please enter a message to send!");const t=new showdown.Converter;e.newMsg.htmlMsg=t.makeHtml(e.newMsg.mdMsg),n.sendMsg(e.newMsg).then(e=>{bulmabox.alert("Message Sent!","Your message is on its way!")})}}]),t.controller("nav-cont",["$scope","$http","$state","$log","userFact",function(e,t,o,s,n){e.currState="dash",e.logout=function(){bulmabox.confirm("Logout","Are you sure you wish to logout?",function(t){if(!t||null==t)return!0;n.logout().then(function(t){s.debug("b4 logout usr removl, parent scope is",e.$parent.user),e.$parent.user=null,s.debug("and now its",e.$parent.user),o.go("appSimp.login")})})},s.debug("USER ON NAVBAR",e.$parent&&e.$parent.user),e.navEls=[{st:"dash",icon:"user-circle",protected:!1,text:"Profile"},{st:"match",icon:"users",protected:!1,text:"Match"},{st:"mentor",icon:"graduation-cap",protected:!1,text:"Mentoring"},{st:"vote",icon:"check-square",protected:!1,text:"Voting"},{st:"mail",icon:"envelope",protected:!1,text:"Mailbox"},{st:"mod",icon:"cogs",protected:!0,text:"Mod Controls"},{st:"help",icon:"question-circle",protected:!0,text:"Help"}],e.goState=t=>{e.currState=t,o.go("app."+t)},e.mobActive=!1}]),s.controller("reset-contr",["$scope","$http","$location","$log","userFact",function(e,t,o,s,n){e.key=window.location.search.slice(5),e.isRf=window.location.href.includes("rf"),e.key||e.isRf||(window.location.href="/rf"),n.resetKey(e.key).then(function(t){s.debug("getting reset user status?",t),e.user=t.data}).catch(t=>{"noUsr"!=t.data||e.isRf||(window.location.href="/rf")}),e.doReset=function(){e.user&&e.pwd&&e.pwdDup&&e.pwdDup==e.pwd&&e.key?n.resetPwd({acct:e.user.user,pwd:e.pwd,pwdDup:e.pwdDup,key:e.key}).then(function(t){"err"==t.data?bulmabox.alert("Error resetting password","There was an error resetting your password. Please contact a mod"):bulmabox.alert("Password Reset","Your password was successfully reset! We're redirecting you to login now.",function(){e.goLogin()})}):bulmabox.alert("Error: Missing data","Make sure you&rsquo;ve reached this page from a password reset link, and that you have entered the same password in both fields!")},e.goLogin=()=>{window.location.href="../../login"}}]);t.controller("vote-cont",["$scope","$http","$state","$log",function(t,o,s,n){t.voteItems=[],t.regetVotes=()=>{o.get("/topic/vote").then(e=>{console.log("topics for voting are",e),t.voteItems=e&&e.data||[]})},t.regetVotes(),t.submitVote=(e,s)=>{o.put("/topic/vote",{id:s._id,mode:e}).then(e=>{t.regetVotes()})},t.getScore=e=>e.votes.votesUp.length-e.votes.votesDown.length,e.on("voteRef",function(e){t.regetVotes()})}]),t.factory("socketFac",["$rootScope",function(e){var t=io.connect();return{on:function(o,s){t.on(o,function(){var o=arguments;e.$apply(function(){s.apply(t,o)})})},emit:function(o,s,n){t.emit(o,s,function(){var o=arguments;e.$apply(function(){n&&n.apply(t,o)})})}}}]),t.run(["$rootScope","$state","$stateParams","$transitions","$q","userFact","$log",function(e,t,o,s,n,r,a){s.onBefore({to:"app.**"},function(e){let o=n.defer();return a.debug("TRANS",e),e.injector().get("userFact").getUser().then(function(e){a.debug("response from login chck",e),e.data?o.resolve(!0):o.resolve(t.target("appSimp.login",void 0,{location:!0}))}).catch(e=>{a.debug("TRANSITION BLOCKED! Error was",e),o.resolve(t.target("appSimp.login",void 0,{location:!0}))}),o.promise}),s.onFinish({to:"app.**"},function(){document.body.scrollTop=document.documentElement.scrollTop=0})}]),t.factory("userFact",["$http","$log",function(e,t){return{getUser:function(){return e.get("/user/usrData").then(function(e){return t.debug("getUser in fac says:",e),e})},newUser:function(t){return e.post("/user/new",t).then(function(e){return e})},login:function(t){return e.put("/user/login",t).then(function(e){return e})},logout:function(){return e.get("/user/logout").then(function(e){return e})},sendMsg:function(t){return e.put("/user/sendMsg",t).then(function(e){return e})},forgot:function(t){return e.put("/user/forgot",t).then(function(e){return e})},resetKey:function(t){return e.get("/user/resetUsr?key="+o).then(function(e){return e})},resetPwd:function(t){return e.put("/user/resetPwd",o).then(function(e){return e})},nameCheck:function(t){return e.get("/user/nameOkay?name="+t).then(function(e){return e})}}}])}();